...
const filteredSales = computed(() => {
  return sales.value.filter((sale) => {
    ...
    if (
      ...
      sale.method !== methodFilter.value
    )
      return false;
    if (
      statusFilter.value &&
      statusFilter.value !== "all" &&
      sale.status !== statusFilter.value
    )
      return false;
    return true;
  });
...
const totalPages = computed(() =>
  Math.max(1, Math.ceil(filteredSales.value.length / pageSize.value)),
);
...
async function reprintSale(sale: any) {
  ...
  const receiptData: ReceiptData = {
    ...
    receiptNumber:
      sale.sale_id?.slice(0, 12).toUpperCase() ||
...
<template>
  <div class="space-y-5">
    ...
    <div
      class="hidden md:block rounded-xl border border-(--ui-border) bg-(--ui-bg-elevated) overflow-hidden"
    >
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-(--ui-border) bg-(--ui-bg-accented)/30">
              <th
                class="text-left py-3 px-4 font-medium text-(--ui-text-dimmed) text-xs uppercase tracking-wider"
              >
                Sale ID
              </th>
              <th
              ...
              <th
                class="text-center py-3 px-4 font-medium text-(--ui-text-dimmed) text-xs uppercase tracking-wider"
              >
                Items
              </th>
              <th
                class="text-right py-3 px-4 font-medium text-(--ui-text-dimmed) text-xs uppercase tracking-wider"
              >
                Total
              </th>
              <th
          ...
          <tbody>
            ...
            >
              ...
              <td class="py-3 px-4 text-(--ui-text-muted) text-xs">
                {{ sale.date }}
              </td>
              <td class="py-3 px-4 text-(--ui-text-highlighted)">
                {{ sale.customer || "Walk-in" }}
              </td>
              <td class="py-3 px-4 text-center text-(--ui-text-muted)">
                {{ sale.items.length }}
              </td>
              <td
                class="py-3 px-4 text-right font-semibold text-(--ui-text-highlighted)"
              >
                {{ format(sale.total) }}
              </td>
              <td class="py-3 px-4 text-center">
                <UBadge
                  :color="methodColors[sale.method] as any"
                  variant="subtle"
                  size="xs"
                  >{{ sale.method }}</UBadge
                >
              ...
              <td class="py-3 px-4 text-center">
                <UBadge
                  :color="statusColors[sale.status] as any"
                  variant="subtle"
                  size="xs"
                  >{{ sale.status }}</UBadge
                >
    ...
    <div class="block md:hidden space-y-3">
      ...
      >
        ...
        <p class="text-xs text-(--ui-text-dimmed) mt-1">
          Transactions completed in the register will show up here
        </p>
      </div>
      ...
      >
        ...
        <div class="flex items-start justify-between gap-2">
          <div>
            <span
              class="font-mono text-xs font-semibold text-(--ui-text-dimmed)"
            >
        ...
        <div
          class="flex items-center justify-between text-xs py-2 px-3 rounded-xl bg-(--ui-bg-accented)/30 border border-(--ui-border)/40"
        >
        ...
        <div
          class="flex items-center justify-between pt-1 border-t border-(--ui-border)/40"
        >
          <div>
            <span
              class="text-[10px] uppercase font-bold text-(--ui-text-dimmed) tracking-wider block"
              >Total Amount</span
            >
            <span
              class="text-base font-black text-green-600 dark:text-green-400 font-mono"
            >
              {{ format(sale.total) }}
            </span>
          </div>
    ...
    >
      <div v-if="selectedSale" class="space-y-4">
        <div
          class="flex items-center justify-between p-3.5 rounded-2xl bg-(--ui-bg-accented)/30 border border-(--ui-border)"
        >
          <div>
          ...
          <UBadge
            :color="statusColors[selectedSale.status] as any"
            variant="subtle"
          >
            {{ selectedSale.status }}
          </UBadge>
        ...
        <div
          class="grid grid-cols-2 sm:grid-cols-3 gap-3 p-3.5 rounded-2xl bg-(--ui-bg-accented)/20 border border-(--ui-border) text-sm"
        >
          <div>
          ...
          <div>
            ...
            <p class="font-medium text-(--ui-text-highlighted) truncate">
              {{ selectedSale.staff?.fullname || "staff" }}
            </p>
          </div>
          <div>
            <p class="text-xs text-(--ui-text-dimmed)">Payment Method</p>
            <UBadge
              :color="methodColors[selectedSale.method] as any"
              variant="subtle"
              size="xs"
              class="mt-1"
            >
        ...
        <div class="space-y-2">
          <p
            class="text-xs font-bold text-(--ui-text-dimmed) uppercase tracking-wider"
          >
            Items Breakdown
          </p>
          <div class="space-y-2">
            ...
            >
              <div>
                <span
                  ...
                  >{{ item.name }}</span
                >
                <span class="text-xs text-(--ui-text-dimmed) ml-2 font-mono"
                  >× {{ item.qty }}</span
                >
              </div>
              <span class="font-bold text-sm text-(--ui-text-highlighted)">{{
        ...
        <div
          class="flex justify-between items-center font-black text-xl p-4 rounded-2xl bg-(--ui-bg-accented)/40 border border-(--ui-border)"
        >
        ...
        <div
          ...
          class="text-sm p-3.5 rounded-xl bg-(--ui-bg-accented)/30 border border-(--ui-border)"
        >
          <p class="text-xs text-(--ui-text-dimmed) mb-1 font-bold">
            Staff Note
          </p>
