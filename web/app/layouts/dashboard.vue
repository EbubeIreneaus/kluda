...
<template>
  <div class="flex h-screen overflow-hidden bg-(--ui-bg)">
      ...
      <header class="h-16 border-b border-(--ui-border) bg-(--ui-bg)/80 backdrop-blur-xl px-4 lg:px-8 flex items-center justify-between shrink-0">
        ...
        <div class="flex items-center gap-3">
          <a :href="posUrl" target="_blank">
            <UButton size="sm" color="primary" variant="soft" class="font-semibold px-3.5 py-2">
    ...
    <USlideover v-model:open="isMobileMenuOpen" side="left" title="Menu">
      <template #body>
        <div class="flex flex-col h-full gap-4">
          <div class="p-3 rounded-2xl bg-(--ui-bg-elevated) border border-(--ui-border) space-y-1.5">
            <div class="text-[11px] font-semibold uppercase tracking-wider text-(--ui-text-muted)">Active Store</div>
            <div v-if="ownerStore.stores.length > 0">
              <select
                :value="ownerStore.selectedStoreId"
                @change="handleStoreChange(($event.target as HTMLSelectElement).value)"
          ...
          <nav class="space-y-1 flex-1">
            <NuxtLink
              ...
              :key="item.to"
              :to="item.to"
              class="flex items-center gap-3 px-3.5 py-3 rounded-xl text-sm font-medium transition cursor-pointer"
              :class="isActive(item.to) ? 'bg-emerald-500/10 text-emerald-500 font-semibold' : 'text-(--ui-text-muted) hover:bg-(--ui-bg-accented) hover:text-(--ui-text)'"
          ...
          <div class="space-y-3 pt-3 border-t border-(--ui-border)">
            <a :href="posUrl" target="_blank" class="block">
              <div class="p-3.5 rounded-2xl bg-gradient-to-br from-emerald-500/10 to-teal-500/10 border border-emerald-500/20 hover:border-emerald-500/40 transition group">
                <div class="flex items-center gap-2 mb-1">
                  <UIcon name="i-lucide-scan-barcode" class="w-4 h-4 text-emerald-500" />
                  <span class="text-xs font-bold text-(--ui-text-highlighted)">POS Cashier App</span>
                </div>
            ...
            <div class="p-3 rounded-2xl bg-(--ui-bg-elevated) border border-(--ui-border) flex items-center justify-between">
              <div class="flex items-center gap-2.5 min-w-0">
                <div class="w-8 h-8 rounded-full bg-emerald-500/20 text-emerald-500 flex items-center justify-center font-bold text-xs shrink-0">
                  {{ ownerStore.initials }}
                </div>
