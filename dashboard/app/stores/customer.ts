...
export const useCustomerStore = defineStore("customers", () => {
  ...
  async function fetchCustomers() {
    try {
      const response = await $fetch<Customer[]>(`${apiBase}/customer/`, {
        method: "GET",
        headers: {
  ...
  async function fetchDebtors() {
    try {
      const response = await $fetch(`${apiBase}/debtor/`, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${auth.token ?? ""}`,
        },
      ...
      if (response && Array.isArray(response)) {
        const mapped: LocalDebtor[] = response.map((d) => ({
  ...
  async function updateCustomer(
    ...
    loading.value = true;
    try {
      await $fetch(`${apiBase}/customer/${customer_id}`, {
        method: "PUT",
        headers: {
  ...
  async function updateDebtor(
    ...
    loading.value = true;
    try {
      await $fetch(`${apiBase}/debtor/${debtor_id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${auth.token ?? ""}`,
