...
class User(Base):
    ...
    user_id: MappedColumn[uuid.UUID] = mapped_column(
        UUID, default=uuid.uuid4, index=True, unique=True
    )
    ...
    access_token: MappedColumn[str | None] = mapped_column(
        String(500), nullable=True, unique=True, index=True
    )
    last_login: MappedColumn[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    otp_token: MappedColumn[str | None] = mapped_column(
        String(255), unique=True, index=True
    )
    otp_expires_at: MappedColumn[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    ...
    notification_subscription: MappedColumn[list["NotificationSubscription"]] = (
    ...
    status: MappedColumn[UserStatus] = mapped_column(
        Enum(UserStatus), default=UserStatus.ACTIVE
    )
    sessions: MappedColumn[list["UserSession"]] = relationship(
        back_populates="user", cascade="all, delete-orphan"
    )
    ...
    paystack_customer_code: MappedColumn[str | None] = mapped_column(
        String(100), unique=True, index=True, nullable=True
    )
    paystack_authorization: MappedColumn[dict[str, Any] | None] = mapped_column(
        JSON, nullable=True
    )

    referral_code: MappedColumn[str | None] = mapped_column(
        String(30), unique=True, index=True, nullable=True
    )
    referred_by_id: MappedColumn[int | None] = mapped_column(
        ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True
    )
    referred_by: MappedColumn["User | None"] = relationship(
        "User", remote_side=[id], foreign_keys=[referred_by_id]
    )

    has_used_trial: MappedColumn[bool] = mapped_column(
...
class StoreMember(Base):
    ...
    store_id: MappedColumn[uuid.UUID] = mapped_column(
        ForeignKey("stores.store_id", ondelete="CASCADE"), index=True
    )
    user_id: MappedColumn[uuid.UUID] = mapped_column(
    ...
    status: MappedColumn[StaffStatus] = mapped_column(
...
class UserSession(Base):
    ...
    id: MappedColumn[int] = mapped_column(Integer, primary_key=True)
    session_id: MappedColumn[uuid.UUID] = mapped_column(
        UUID, default=uuid.uuid4, unique=True, index=True
    )
    user_id: MappedColumn[uuid.UUID] = mapped_column(
        UUID, ForeignKey("users.user_id", ondelete="CASCADE"), nullable=False
    )
    refresh_token_hash: MappedColumn[str] = mapped_column(
        String(64), nullable=False, unique=True, index=True
    )
    previous_refresh_token_hash: MappedColumn[str | None] = mapped_column(
    ...
    expired_at: MappedColumn[datetime] = mapped_column(
...
class Customer(Base):
    ...
    id: MappedColumn[int] = mapped_column(Integer, primary_key=True)
    customer_id: MappedColumn[uuid.UUID] = mapped_column(
    ...
    email: MappedColumn[EmailStr | None] = mapped_column(String, nullable=True)
    debts: MappedColumn[list["Debt"]] = relationship(
        back_populates="customer", cascade="all, delete-orphan"
    )
    deleted: MappedColumn[bool] = mapped_column(Boolean, default=False, nullable=True)
    deleted_at: MappedColumn[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    status: MappedColumn[CustomerStatus] = mapped_column(
...
class Debt(Base):
    ...
    store: MappedColumn["Store"] = relationship()
    amount: MappedColumn[int] = mapped_column(Integer)
    note: MappedColumn[str | None] = mapped_column(Text, nullable=True)
    status: MappedColumn[Literal["paid", "unpaid"]] = mapped_column(
        String(10), default="unpaid"
    )
...
class NotificationSubscription(Base):
    ...
    user: MappedColumn["User"] = relationship(
        back_populates="notification_subscription"
    )
