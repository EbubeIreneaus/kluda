...
async def execute_demo_reset(
    ...
    store = await db.scalar(select(Store).options(selectinload(Store.user)).where(Store.store_id == store_id))
    if not store:
