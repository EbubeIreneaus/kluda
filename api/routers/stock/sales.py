...
from models.user import Customer, Debtor, Staff
...
async def ping(db: AsyncSession = Depends(get_db)):
    try:
        # Check connection is active by doing a count query
        res = await db.execute(select(func.count()).select_from(Stock))
        count = res.scalar()
...
async def create_sales_batch(
    ...
    try:
        for sale_data in sales_data:
            # 1. Check idempotency
            stmt = select(Sale).where(Sale.idempotency_key == sale_data.idempotency_key)
            existing_sale = (await db.execute(stmt)).scalar_one_or_none()
            if existing_sale:
                continue  # Skip already processed sale

            # 2. Check customer
            customer = None
            if sale_data.customer_id:
                c_res = await db.execute(select(Customer).where(Customer.customer_id == sale_data.customer_id))
                customer = c_res.scalar_one_or_none()
                if not customer:
                    raise HTTPException(
                        status_code=status.HTTP_404_NOT_FOUND,
                        detail=f"Customer with ID '{sale_data.customer_id}' not found",
                    )

            # 3. Create Sale
            # sale_id will be generated by database defaults (UUID)
            new_sale = Sale(
            ...
            # Fetch generated sale_id
            sale_id = new_sale.sale_id

            # 4. Add items and update inventory stock
            items_to_add = []
            note_parts = []
            grand_total = 0

            for item_in in sale_data.items:
                stk_res = await db.execute(select(Stock).where(Stock.slug == item_in.stock_slug))
                stock_item = stk_res.scalar_one_or_none()
                if not stock_item:
                    raise HTTPException(
                        status_code=status.HTTP_404_NOT_FOUND,
                        detail=f"Stock product '{item_in.stock_slug}' not found",
                    )

                # Format for debt note: e.g. "3 piece of Peak Milk Powder 400g"
                qty_str = str(int(item_in.quantities)) if item_in.quantities.is_integer() else str(item_in.quantities)
                note_parts.append(f"{qty_str} {stock_item.unit_in} of {stock_item.name}")

                # Accumulate grand total
                grand_total += int(item_in.amount * item_in.quantities)

                # Deduct quantity if sale completed
                if sale_data.status == "completed":
                    stock_item.quantities = max(0.0, float(stock_item.quantities) - float(item_in.quantities))

                sale_item = SaleItem(
                    sale_id=sale_id,
                    stock_slug=item_in.stock_slug,
        ...
        return {"success": True, "created_count": len(created_ids), "created_ids": created_ids}

    except Exception as e:
        await db.rollback()
        logging.error(f"Error creating sales batch: {str(e)}")
...
async def get_analytics(
    ...
    date_from: date | None = Query(default=None, description="Start date (YYYY-MM-DD) when period=custom"),
    date_to: date | None = Query(default=None, description="End date (YYYY-MM-DD) when period=custom"),
    db: AsyncSession = Depends(get_db),
    ...
    top_products = [
        {"name": row.name, "qty": float(row.total_qty), "revenue": int(row.total_revenue)}
        for row in top_res
    ]
    ...
    for row in rev_series_res:
        day = str(row.day)
        daily_revenue[day] = daily_revenue.get(day, 0) + max(0, int(row.subtotal) - int(row.discount))
    revenue_series = [{"date": d, "revenue": v} for d, v in sorted(daily_revenue.items())]
...
async def get_sales(
    ...
    stmt = (
        ...
        .where(and_(Sale.created_at >= day_start, Sale.created_at < day_end))
