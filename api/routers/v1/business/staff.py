...
async def generate_staff_id(db: AsyncSession, prefix="STF") -> str:
    for _ in range(10):
        code = None
        while True:
            code = f"{prefix}{secrets.randbelow(8999) + 1000}"
...
async def reset_access_token(
    ...
    res = await db.execute(
        select(Staff).where(
            Staff.staff_id == target_staff_id, Staff.store_id == store.store_id
        )
    )
...
async def get_staffs(
    ...
    if status_filter:
        query = query.where(
            Staff.status == status_filter, Staff.store_id == store.store_id
        )
...
async def get_staff(
    ...
    res = await db.execute(
        select(Staff).where(
            Staff.staff_id == staff_id, Staff.store_id == store.store_id
        )
    )
    staff = res.scalar_one_or_none()
...
async def update_staff(
    ...
    if staff.status == StaffStatus.TERMINATED:
        raise HTTPException(
            ...
            detail=f"Staff account is terminated and cannot be managed.",
        )
        
    _status = update_data.status if update_data.status else None
    ...
    if "permission" in values and values["permission"] is not None:
        values["permission"] = [
            p.value if hasattr(p, "value") else str(p) for p in values["permission"]
        ]
    
    if values:
        stmt = update(Staff).values(**values).where(Staff.staff_id == staff_id)
        if _status and _status == StaffStatus.TERMINATED:
            stmt.values(session=[], access_token=None)
