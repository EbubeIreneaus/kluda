...
async function fetchMailboxes() {
  try {
    ...
    if (mailboxes.value.length > 0 && !selectedMailbox.value) {
      selectedMailbox.value = mailboxes.value[0]
      composeForm.value.mailbox_id = mailboxes.value[0].mailbox_id
      await fetchThreads()
    }
  } catch {
    mailboxes.value = []
...
async function handleSendReply() {
  ...
  try {
    const newMsg = await apiFetch<any>(`/admin/inbox/threads/${selectedThread.value.thread_id}/reply`, {
      method: 'POST',
...
async function setThreadStatus(st: string) {
  ...
  try {
    await apiFetch(`/admin/inbox/threads/${selectedThread.value.thread_id}/status?status_val=${st}`, {
      method: 'PUT'
    })
...
<template>
  <div class="h-[calc(100vh-65px)] md:h-screen flex flex-col md:flex-row min-w-0 bg-zinc-950 text-zinc-100 overflow-hidden">
    <div class="w-full md:w-88 border-r border-zinc-800/80 bg-zinc-900/40 flex flex-col shrink-0 h-full">
      <div class="p-4 border-b border-zinc-800 flex flex-col gap-3">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            <span class="text-sm font-bold text-white">Support Email Desk</span>
          </div>
          <UButton
            label="Compose"
        ...
        <div class="flex flex-col gap-1">
          <label class="text-[11px] font-medium text-zinc-400">Active Mailbox</label>
          <select
            v-model="selectedMailbox"
            class="w-full bg-zinc-950 border border-zinc-800 text-xs rounded-lg px-3 py-1.5 text-zinc-200 focus:outline-none focus:border-emerald-500"
          >
            <option v-for="mb in mailboxes" :key="mb.mailbox_id" :value="mb">
              {{ mb.name }} ({{ mb.email }})
            </option>
          </select>
        </div>

        <div class="flex bg-zinc-950 p-1 rounded-lg border border-zinc-800 gap-0.5 overflow-x-auto">
          <button
            v-for="f in folderTabs"
            :key="f.id"
            :class="[
              'px-2.5 py-1 text-[11px] rounded-md font-medium flex items-center gap-1 transition-colors shrink-0',
              selectedFolder === f.id ? 'bg-zinc-800 text-emerald-400 font-bold shadow-xs' : 'text-zinc-400 hover:text-zinc-200'
            ]"
      ...
      <div class="flex-1 overflow-y-auto divide-y divide-zinc-800/50">
        <div v-if="isLoadingThreads" class="p-6 text-center text-xs text-zinc-500">
          Loading messages...
        </div>
        <div v-else-if="threads.length === 0" class="p-6 text-center text-xs text-zinc-500">
        ...
        <div
          ...
          :class="[
            ...
            selectedThread?.thread_id === t.thread_id ? 'bg-zinc-800/60 border-l-2 border-emerald-500' : 'hover:bg-zinc-800/30'
          ]"
          @click="selectThread(t)"
        >
          <div class="flex items-center justify-between">
            <span class="text-xs font-semibold text-zinc-200 truncate">{{ t.customer_email }}</span>
            <span class="text-[10px] text-zinc-400 shrink-0">{{ new Date(t.last_message_at).toLocaleDateString() }}</span>
          </div>
          <div class="text-xs text-zinc-300 font-medium truncate">{{ t.subject }}</div>
          <div class="text-[11px] text-zinc-400 line-clamp-2">{{ t.snippet || 'No preview' }}</div>
        </div>
      </div>
    </div>

    <div v-if="selectedThread" class="flex-1 flex flex-col h-full min-w-0 bg-zinc-950">
      <div class="p-4 border-b border-zinc-800 flex items-center justify-between bg-zinc-900/60 backdrop-blur-sm">
        <div class="min-w-0">
          <h2 class="text-sm font-bold text-white truncate">{{ selectedThread.subject }}</h2>
          <div class="text-xs text-zinc-400 mt-0.5 flex items-center gap-2">
            <span>Customer: {{ selectedThread.customer_email }}</span>
            <span>•</span>
            <span class="font-mono text-[11px] text-emerald-400">Mailbox: {{ selectedThread.to }}</span>
          </div>
        </div>
      ...
      <div class="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
        ...
        >
          <div class="flex items-center justify-between gap-4 text-[10px] text-zinc-400">
            <div class="flex items-center gap-1.5">
              <span :class="['px-1.5 py-0.2 rounded text-[9px] font-bold uppercase', msg.direction === 'outgoing' ? 'bg-emerald-500/20 text-emerald-300' : 'bg-blue-500/20 text-blue-300']">
                {{ msg.direction }}
              </span>
          ...
          <div class="prose prose-invert prose-xs max-w-none text-zinc-100" v-html="msg.body" />
        </div>
      </div>

      <div class="p-4 border-t border-zinc-800 bg-zinc-900/60 flex flex-col gap-3">
        <textarea
          v-model="replyText"
          rows="3"
        ...
        <div class="flex justify-between items-center">
          <span class="text-[11px] text-zinc-400">Replying from {{ selectedThread.to || adminUser?.company_email }}</span>
          <UButton
            label="Send Reply"
            icon="i-lucide-send"
            color="primary"
    ...
    <div v-else class="flex-1 flex items-center justify-center text-zinc-500 text-xs">
          ...
          <div class="flex flex-col gap-3.5 overflow-y-auto pr-1.5 py-3 flex-1 min-h-0">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div class="flex flex-col gap-1">
                ...
                <UInput v-model="composeForm.to_email" placeholder="client@example.com or user@kluda.app" size="sm" />
              </div>
            </div>
